...
public class StructureTombezior extends ElementsAssassincrownMod.ModElement {
	public StructureTombezior(ElementsAssassincrownMod instance) {
		super(instance, 93);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassincrown", "ezio_tomb_(relevations)"));
				if (template == null)
					return;
