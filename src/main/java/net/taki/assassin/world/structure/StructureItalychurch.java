...
public class StructureItalychurch extends ElementsAssassincrownMod.ModElement {
	public StructureItalychurch(ElementsAssassincrownMod instance) {
		super(instance, 149);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 1200) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassincrown", "italian_church"));
				if (template == null)
					return;
