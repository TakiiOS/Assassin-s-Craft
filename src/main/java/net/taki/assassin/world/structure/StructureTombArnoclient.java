...
public class StructureTombArnoclient extends ElementsAssassincrownMod.ModElement {
	public StructureTombArnoclient(ElementsAssassincrownMod instance) {
		super(instance, 97);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassincrown", "arno_tomb"));
				if (template == null)
					return;
