...
public class StructureEstructuraprueba extends ElementsAssassincrownMod.ModElement {
	public StructureEstructuraprueba(ElementsAssassincrownMod instance) {
		super(instance, 90);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassincrown", "altair_tomb"));
				if (template == null)
					return;
