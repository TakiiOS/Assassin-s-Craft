...
public class StructureGreatwatchtower extends ElementsAssassincrownMod.ModElement {
	public StructureGreatwatchtower(ElementsAssassincrownMod instance) {
		super(instance, 191);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 3000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("assassincrown:syrianplains")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("assassincrown:syrianlands")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassincrown", "great_watchtower"));
				if (template == null)
					return;
