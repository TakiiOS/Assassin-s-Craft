...
public class StructureItalywatchtower extends ElementsAssassincrownMod.ModElement {
	public StructureItalywatchtower(ElementsAssassincrownMod instance) {
		super(instance, 148);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2000) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassincrown", "italian_watchtower"));
				if (template == null)
					return;
