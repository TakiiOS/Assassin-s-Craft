...
public class StructureTombedwardclient extends ElementsAssassincrownMod.ModElement {
	public StructureTombedwardclient(ElementsAssassincrownMod instance) {
		super(instance, 95);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassincrown", "edward_tomb"));
				if (template == null)
					return;
