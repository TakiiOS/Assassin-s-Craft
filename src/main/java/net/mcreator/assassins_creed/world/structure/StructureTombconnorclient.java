...
public class StructureTombconnorclient extends ElementsAssassinsCraftMod.ModElement {
	public StructureTombconnorclient(ElementsAssassinsCraftMod instance) {
		super(instance, 94);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassins_craft", "connor_tomb"));
				if (template == null)
					return;
