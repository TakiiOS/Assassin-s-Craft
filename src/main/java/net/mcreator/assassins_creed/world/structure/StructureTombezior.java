...
public class StructureTombezior extends ElementsAssassinsCraftMod.ModElement {
	public StructureTombezior(ElementsAssassinsCraftMod instance) {
		super(instance, 93);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassins_craft", "ezio_tomb_(relevations)"));
				if (template == null)
					return;
