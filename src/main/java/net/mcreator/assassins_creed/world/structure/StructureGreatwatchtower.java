...
public class StructureGreatwatchtower extends ElementsAssassinsCraftMod.ModElement {
	public StructureGreatwatchtower(ElementsAssassinsCraftMod instance) {
		super(instance, 191);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 3000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				require = Blocks.AIR.getDefaultState();
				if (blockAt.getBlock() == require.getBlock())
					blockCriteria = true;
				if (!blockCriteria)
					continue;
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("assassins_craft:syrianplains")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("assassins_craft:syrianlands")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassins_craft", "great_watchtower"));
				if (template == null)
					return;
