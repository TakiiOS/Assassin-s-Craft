...
package net.mcreator.assassins_creed.world.structure;
...
import net.mcreator.assassins_creed.world.WorldThethirdcrusade1189;
import net.mcreator.assassins_creed.world.WorldItaly1459;
import net.mcreator.assassins_creed.ElementsAssassinsCraftMod;

import java.util.Random;

@ElementsAssassinsCraftMod.ModElement.Tag
public class StructureNormal_watchtower extends ElementsAssassinsCraftMod.ModElement {
	public StructureNormal_watchtower(ElementsAssassinsCraftMod instance) {
		super(instance, 146);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 4000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("assassins_craft:nomanlands")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("assassins_craft:italianplains")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("assassins_craft:syrianplains")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassins_craft", "basic_watchtower"));
				if (template == null)
					return;
