...
public class StructureTombArnoclient extends ElementsAssassinsCraftMod.ModElement {
	public StructureTombArnoclient(ElementsAssassinsCraftMod instance) {
		super(instance, 97);
	}
		...
		if ((random.nextInt(1000000) + 1) <= 2750) {
			...
			for (int a = 0; a < count; a++) {
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("assassins_craft", "arno_tomb"));
				if (template == null)
					return;
